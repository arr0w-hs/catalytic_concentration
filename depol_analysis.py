...
from base_transform import schmidt_decomp_of_dm1
from base_depol_channels import  new_state_pauli_x1, new_state_pauli_x2
import numpy as np
import pandas as pd
...
#     prob_in_state = 1-0.3*i/n #0.95
#     alpha = 0.99#1 - j/m*0.1

#     final_state = new_state_pauli_x1(alpha, prob_in_state)

#     rr = schmidt_decomp_of_dm1(final_state)
...
#     eigen_val_list.append(1-rr[4])


#     #z = np.polyfit(prob_in_state_list, eigen_val_list, 2)
#     # p = np.poly1d(z)
...
# #z_list.append(z[0])
# #print(z)
# z = np.polyfit(fid_nocat_list,prob_in_state_list, 2)
# p = np.poly1d(z)
# plt.figure()
# plt.plot(eigen_val_list, fid_nocat_list)
# #plt.plot(fid_nocat_list,p(fid_nocat_list))
# plt.ylabel("Infidelity of SLOCC")
# plt.xlabel("1-maximum eigenvalue")
# plt.legend([str(z)])
# plt.grid()

# z = np.polyfit(prob_in_state_list, fid_nocat_list, 2)
# p = np.poly1d(z)
# plt.figure()
# plt.plot(prob_in_state_list, fid_nocat_list)
...
# p = np.poly1d(z)
# plt.figure()
# plt.plot(prob_in_state_list, eigen_val_list)
# plt.xlabel("Prob of error")
# plt.ylabel("1 - maximum eigenvalue")
# plt.legend([str(z)])
# # plt.grid()
...
#         fid_nocat, prob_nocat, post_locc_state_nocat, _ = non_catalytic_conversion(final_state)
#         fid2.append(1-fid_nocat)
#         #prob2.append(1-prob_in_state)
#         eigen_val_list.append(1-rr[4])
...
# plt.figure()
# #plt.plot(prob_in_state_list, fid_nocat_list)
# plt.plot(prob_in_state_list, fid2)
#     #z = np.polyfit(prob_in_state_list, eigen_val_list, 2)
#     # p = np.poly1d(z)
