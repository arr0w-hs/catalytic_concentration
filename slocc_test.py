...
from base_catalyst import prob_of_transformation2
...
def concat_zeros(out_state, in_state):
    ...
    if np.shape(out_state)[0] > 1:
        if shape_in_state >= shape_out_state:
            extra_zeros = np.zeros(shape_in_state - shape_out_state)
            out_state = np.concatenate((out_state, extra_zeros), axis=None)
        else:
            raise Exception("Incoherent dimensions of states")
    else:
        raise Exception("product state")
...
def majorisation_check(final_state, initial_state):
    ...
    probab = prob_of_transformation2(final_state, initial_state)
    if probab < 0.999:
